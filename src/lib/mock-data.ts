...
export const mockDashboardKpis = kpiMetrics.map(m => ({
  title: m.title,
  value: m.value,
  trend: { value: parseFloat(m.change), label: `${m.change} vs last month`, isPositive: m.trend === 'up' },
  icon: m.icon.toLowerCase(),
}));

export const mockAlerts = executiveAlerts.map(a => ({
  id: a.id.toString(),
  title: a.title,
  description: a.description,
  level: a.type === 'error' ? 'critical' : a.type,
  timestamp: a.time,
  category: a.category,
  isResolved: false,
}));

export const mockActionItems = actionItems.map(a => ({
  id: a.id.toString(),
  title: a.title,
  description: a.description,
  priority: a.priority,
